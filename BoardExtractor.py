...
class BoardExtractor:
    ...
    def detect_and_crop_grid(self):
        ...
        for i in range(len(lines)):
            tmpimg = drawLine(lines[i], tmpimg)
        try:
        ...
        twppp = drawLine(leftedge, tmppp)
        twppp = drawLine(rightedge, twppp)
        twppp = drawLine(topedge, twppp)
        twppp = drawLine(bottomedge, twppp)
        ...
        #Next interesection of these four lines
        ...
        cv2.circle(tmppp, (ptTopLeft[0], ptTopLeft[1]), 5, 0, -1)
        cv2.circle(tmppp, (ptTopRight[0], ptTopRight[1]), 5, 0, -1)
        cv2.circle(tmppp, (ptBottomLeft[0],ptBottomLeft[1]), 5, 0, -1)
        cv2.circle(tmppp, (ptBottomRight[0], ptBottomRight[1]), 5, 0, -1)
        try:
    ...
    def create_image_grid(self):
        ...
        #Adaptive thresho,ding the cropped grid and inverting it
